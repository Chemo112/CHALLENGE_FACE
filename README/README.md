...
## <a name="_4bff8fwqm8ud"></a>**Contributions**
Noi siamo Maja, Juana, Mattia e Barbara e abbiamo fatto una challenge di face recognition in machine learning per l'università. Dobbiamo scrivere un paragrafo di come ci siamo divisi i compiti tra scrittura e implementaizione dell'algoritmo e scrittura del paper. Vogliamo dividere equamente le task in modo da far emergere che ognuno di noi ha dato il proprio contributo. In realtà per il progetto abbiamo utilizzando modelli pretrainati come MTCNN, YOLO; ArcFace, FaceNet e il dataset di prova ci è stato fornito dal professore (quindi non era di nostra competenza)
